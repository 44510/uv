...
async fn do_lock(
    ...
    let requires_python = if let Some(requires_python) = requires_python {
        if requires_python.is_unbounded() {
            ...
            warn_user_once!("The workspace `requires-python` value (`{requires_python}`) does not contain a lower bound. Include a lower bound to indicate the minimum compatible Python version (e.g., `{default}`).");
